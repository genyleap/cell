...
Types::OptionalString TimestampConverter::getRelativeTime(std::time_t unixTime)
{
    ...
    if (timeDifference < 0) {
        ...
        return safeFormat()->print(safeTranslate(lcode,"core", "hour"), hours, (hours > 1 ? pluralWord : ""));
    } else if (timeDifference < 604800) {
        int days = timeDifference / 86400;
        return safeFormat()->print(safeTranslate(lcode,"core", "day"), days, (days > 1 ? pluralWord : ""));
    } else if (timeDifference < 2592000) {
        int weeks = timeDifference / 604800;
        return safeFormat()->print(safeTranslate(lcode,"core", "week"), weeks, (weeks > 1 ? pluralWord : ""));
    } else if (timeDifference < 31536000) {
        int months = timeDifference / 2592000;
        return safeFormat()->print(safeTranslate(lcode,"core", "month"), months, (months > 1 ? pluralWord : ""));
    } else {
        int years = timeDifference / 31536000;
        return safeFormat()->print(safeTranslate(lcode,"core", "year"), years, (years > 1 ? pluralWord : ""));
    }
}
...
Types::OptionalString TimestampConverter::getTimeDuration(std::chrono::seconds start, std::chrono::seconds end)
{
    ...
    std::stringstream ss;
    if (days > 0)
        ss << days << safeFormat()->print(safeTranslate(lcode,"core", "day"), days, (days > 1 ? pluralWord : "")) << ", ";
    if (hours > 0)
