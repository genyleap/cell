...
Language::Language(const Language& rhs) : m_languageStruct(rhs.m_languageStruct)
{
    ...
    if(config->Setting().getValueMulti(CELL_LANGS, v))
    {
        for(const auto& c : v)
        {
            lstatus = Engine::self().meta()->returnJsonAt(c,"status").asBool();
            if(lstatus == true) {
                lstr = Engine::self().meta()->returnJsonAt(c, std::string("l")).asString();
...
Language::Language(const std::string& uri)
{
    ...
    if(config->Setting().getValueMulti(CELL_LANGS, v))
    {
        for(const auto& c : v)
        {
            viewStr = Engine::self().meta()->returnJsonAt(c, "l").asString();
...
std::string Language::getLanguageCode() __cell_const_noexcept
{
    ...
    {
        default_lang = Engine::self().meta()->returnJsonAt(jvalue).asString();
    }
    if(config->Setting().getValueMulti(CELL_LANGS, v))
    {
        for(const auto& c : v)
        {
            viewUri     =   Engine::self().meta()->returnJsonAt(c, "uri").asString();
            viewName    =   Engine::self().meta()->returnJsonAt(c, "code").asString();
            viewCode    =   Engine::self().meta()->returnJsonAt(c, "l").asString();
            viewStatus  =   Engine::self().meta()->returnJsonAt(c, "status").asBool(); //! Status of language.
