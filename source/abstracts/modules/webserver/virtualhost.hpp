 ...
 *
 * @details EventLoopType is an enum type used to specify the type of event loop used by a server or client.
 * An event loop is an essential component of an I/O-based system, and its purpose is to listen for and respond to incoming events, such as data that has been received from a client or a timer that has expired.
 *
 * In the context of a server or client, an event loop is responsible for handling all I/O operations, such as accepting incoming connections, sending and receiving data, and closing connections.
 * The type of event loop used by a server or client can have a significant impact on its performance and scalability.
 *
 * The EventLoopType enum provides three possible values: Select, Poll, and Epoll, which represent different implementations of the event loop.
 * Select and Poll are generally considered to be less performant than Epoll, particularly when dealing with a large number of connections.
 *
 * Select and Poll are both based on the select() system call, which allows a process to monitor multiple file descriptors (sockets in this case) for I/O readiness. Epoll, on the other hand, is based on the epoll() system call, which provides more efficient I/O event notification and is generally considered to be the fastest of the three event loop types.
 *
 * In summary, the EventLoopType enum is used to specify the type of event loop used by a server or client, which can have a significant impact on its performance and scalability.
 *
 */
...
{
public:
    CELL_DEFAULT_INTERFACE_OCTORS(VirtualHostConfig)
